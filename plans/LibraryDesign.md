...
**Background:** In the Translator consortium, a set of tools have been developed to support the Translator overall infrastructure, including separate KGs, centralized tier 0 graph, TRAPI standard, Biolink tools, Name Resolver, Node Normalizer, Node Annotator, query functions, pathfinder function etc. All the tools can be re-usable for internal developers and super-users for the Translator project. We have currently developed a Translator Component Toolkit (TCT), which is a single repo that provides access to all the components necessary for super users.
...
3. TCT will incorporate TOM (when it becomes mature), Translator_SDK and other utilities as a python library. (TCT will serve as a public library to engage with super users) (Proposed key developers: Guangrong, Yue, Gaurav, Chunlei, +other contributors)  
